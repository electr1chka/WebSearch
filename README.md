...
Поточний MVP вже вміє витягувати списки товарів із OLX, Prom/Bigl, Hotline, Zabros та Daiwa Ukraine, а для Flagman використовує search API з подальшим JSON-LD extraction товарних сторінок. Для generated search pages без підтриманого list extractor він не показує випадкову першу ціну зі сторінки. Rozetka через простий HTTP часто віддає Cloudflare challenge, тому для неї потрібен browser/human-in-the-loop режим або зовнішній search/extraction provider.
